...
It uses `pydantic-ai` and `pydantic-ai-skills` for agent behavior, PostgreSQL for persistence, and Terraform for infrastructure.
...
- `src/agents_party/domain/`: domain models and business concepts that should not depend on Slack or database SDK details
- `src/agents_party/repositories/`: persistence-facing interfaces and repository logic
- `src/agents_party/infrastructure/postgres/`: PostgreSQL-specific persistence implementation
...
- Keep SQLAlchemy / SQLModel database access inside `src/agents_party/infrastructure/postgres/` unless there is a strong reason not to.
...
- Manage relational schema changes with Alembic in this project.
...
- Use the repository `review-agent` for that review step.
- If the current session policy requires explicit user permission before sub-agent delegation, obtain that permission and then run the repository `review-agent`.
