...
Status: Link repository and tool contracts implemented for OSA-3; external sync jobs and Slack UX remain proposed.
...
実装済み repository / gateway interface:

- `GoogleCalendarReadGateway.getEvent(context, { calendarId, eventId })`
- `GoogleCalendarReadGateway.searchEvents(context, query)`
- `PostgresWorkItemRepository.linkCalendarEvent(...)`
- `PostgresWorkItemRepository.unlinkCalendarEvent(...)`
