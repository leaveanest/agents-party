...
Status: Read gateway implemented for OSA-4; write and sync behavior remains proposed.
...
初期 read gateway interface:

- `listCalendars(context: GoogleCalendarUserContext): Promise<GoogleCalendarCalendar[]>`
- `getEvent(context: GoogleCalendarUserContext, input: { calendarId: string; eventId: string }): Promise<GoogleCalendarEvent>`
- `searchEvents(context: GoogleCalendarUserContext, query: GoogleCalendarEventQuery): Promise<GoogleCalendarEvent[]>`
- `listUpcomingEvents(context: GoogleCalendarUserContext, input?: { calendarId?: string; limit?: number; timeMin?: Date | string }): Promise<GoogleCalendarEvent[]>`

`GoogleCalendarUserContext` は `teamId`、`slackUserId`、任意の `googleAccountSubject` を持つ。
接続 lookup と token 復号は infrastructure 実装側で行い、agent 側に Google API details を渡さない。
...
接続情報の PostgreSQL 配置は、既存 OAuth 接続テーブルを使う。

```text
google_auth_connections (team_id, slack_user_id, google_account_subject)
```
...
OSA-4 の read gateway は refresh をまだ実装しない。
期限切れ、失効、復号失敗、壊れた接続レコードは deterministic な reconnect-required error として返す。

将来 refresh を追加する場合も、**常時バックグラウンド更新は行わず、必要時 refresh を標準**とする。

理由:

- Google の access token は通常短命で、`expires_in` から有効期限を見積もれる
- 一方で refresh token 自体はユーザー操作やポリシー変更で失効しうる
- refresh を追加する場合は agent 実行時にだけ試行すれば十分で、運用負荷も小さい
