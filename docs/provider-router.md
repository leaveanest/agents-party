...
Application bootstrap defaults are outside `ProviderRouter.resolveModel`. `loadSettings` may supply `AgentRunner` with a local/bootstrap `agentModelId` so a developer can run the app before workspace, channel, or thread routing records exist. Production-like runtimes must set `AGENT_MODEL`, `APP_DATABASE_BACKEND=postgres`, `DATABASE_URL`, and `LLM_API_KEY_ENCRYPTION_KEY`; missing production configuration fails during settings loading instead of silently choosing a provider or falling back to process-level provider API keys.
