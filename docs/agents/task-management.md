...
`needs_attention` 系の一覧を Firestore で queryable にするために使う。
...
source of truth と read model は workspace 配下に置く。

```text
workspaces/{team_id}/work_items/{work_item_id}
workspaces/{team_id}/work_items/{work_item_id}/participants/{user_id}
workspaces/{team_id}/work_items/{work_item_id}/events/{event_id}
workspaces/{team_id}/attention_index/{user_id}/work_items/{work_item_id}
```
